...
@section('content')
    ...
    <div class="row align-items-stretch">
        <div class="col-md-4 col-lg-3">
            <form action="{{ route('supervisor.leads.details', ['id' => $lead->id]) }}" method="POST">
                @csrf
                <div class="card h-100-int">
                    <div class="card-body border-top border-primary rounded border-3 ">
                        <div class="text-right"><a href="{{ route('supervisor.leads.update', ['id' => $lead->id]) }}"
                                ...
                                <div class="card">
                                    ...
                                    @endswitch">
                                        <div class="row">
                                            ...
                                            <div class="col-2">
                                                <a href="{{ route('supervisor.leads.activityDetailsModal', ['id' => $task->id]) }}"
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="col-3 text-right">
                                                ...
                                                <a href="{{ route('supervisor.leads.activityDetailsModal', ['id' => $activityLog->fk_activity]) }}"
                                                    class="btn btn-outline-primary activity-details">Details</a>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="card h-100 d-inline-block">
                        <div class="card-body p-0 text-center" id="action-buttons-container">
                            <a class="btn btn-special-primary activityModal"
                                href="{{ route('supervisor.leads.activityModal', ['idLead' => $lead->id, 'type' => 1]) }}"
                                data-toggle="tooltip" data-placement="left" title="Notes">
                                <i class="far fa-address-book"></i>
                            </a>
                            <a class="btn btn-special-primary activityModal"
                                href="{{ route('supervisor.leads.activityModal', ['idLead' => $lead->id, 'type' => 2]) }}"
                                data-toggle="tooltip" data-placement="left" title="Email">
                                <i class="far fa-envelope"></i>
                            </a>
                            <a class="btn btn-special-primary activityModal"
                                href="{{ route('supervisor.leads.activityModal', ['idLead' => $lead->id, 'type' => 3]) }}"
                                data-toggle="tooltip" data-placement="left" title="Call">
                                <i class="fas fa-phone-volume"></i>
                            </a>
                            <a class="btn btn-special-primary activityModal"
                                href="{{ route('supervisor.leads.activityModal', ['idLead' => $lead->id, 'type' => 4]) }}"
                                data-toggle="tooltip" data-placement="left" title="Meet">
                                <i class="fas fa-users"></i>
                            </a>
                            <a class="btn btn-special-primary activityModal"
                                href="{{ route('supervisor.leads.activityModal', ['idLead' => $lead->id, 'type' => 5]) }}"
    ...
    @if ($errors->addActivityForm->any())
        @include('supervisor_activities.partials.activityModal', [
            'type' => $typeActivity,
            'idLead' => $lead->id,
        ])
    @endif
    ...
    @if ($errors->detailsActivityForm->any() || $activitySelected !== null)
        @include('supervisor_activities.partials.activityDetailsModal', [
