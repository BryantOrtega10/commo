        ...
        <form action="{{ route('products.update',['id' => $product->id]) }}" method="post">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('description') is-invalid @enderror" id="description" name="description" placeholder="Plan Description:" value="{{ old('description',$product->description) }}">
                            @error('description')
                        ...
                        <div class="form-group">
                            ...
                            <select id="carrier" name="carrier" class="form-control @error('carrier') is-invalid @enderror">
                                ...
                                @foreach ($carriers as $carrier)
                                    <option value="{{$carrier->id}}" @if (old('carrier',$product->fk_carrier) == $carrier->id) selected @endif>{{$carrier->name}}</option>
                                @endforeach                                
                            </select>
                            @error('carrier')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group">
                            ...
                            <select id="business_segment" name="business_segment" class="form-control @error('business_segment') is-invalid @enderror">
                                ...
                                @foreach ($business_segments as $business_segment)
                                    <option value="{{$business_segment->id}}" @if (old('business_segment',$product->fk_business_segment) == $business_segment->id) selected @endif>{{$business_segment->name}}</option>
                                @endforeach                                
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select id="product_type" name="product_type" class="form-control @error('product_type') is-invalid @enderror">
                                ...
                                @foreach ($product_types as $product_type)
                                    <option value="{{$product_type->id}}" @if (old('product_type',$product->fk_product_type) == $product_type->id) selected @endif>{{$product_type->name}}</option>
                                @endforeach                                
                            </select>
                            @error('product_type')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group">
                            ...
                            <select id="plan_type" name="plan_type" class="form-control @error('plan_type') is-invalid @enderror">
                                ...
                                @foreach ($plan_types as $plan_type)
                                    <option value="{{$plan_type->id}}" @if (old('plan_type',$product->fk_plan_type) == $plan_type->id) selected @endif>{{$plan_type->name}}</option>
                                @endforeach                                
                            </select>
                            @error('plan_type')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group">
                            ...
                            <select id="tier" name="tier" class="form-control @error('tier') is-invalid @enderror">
                                ...
                                @foreach ($tiers as $tier)
                                    <option value="{{$tier->id}}" @if (old('tier',$product->fk_tier) == $tier->id) selected @endif>{{$tier->name}}</option>
                                @endforeach                                
                            </select>
                            @error('tier')
                                <span class="invalid-feedback" role="alert">
                ...
                <div class="alias-cont">
                    @foreach (old("alias",$product->product_alias) as $index => $alias_item)
                        <div class="row align-items-end alias-item" data-id="{{$index}}">
                            <div class="col-md-6 col-8">
                                <div class="form-group">
                                    <label for="alias_{{$index}}">Alias {{$index + 1}}:</label>
                                    <input type="text" class="form-control @error("alias.".$index) is-invalid @enderror" id="alias_{{$index}}" name="alias[]" value="{{ old("alias.".$index, isset($product->product_alias[$index]) ? $product->product_alias[$index]->alias : null)}}">
                                    @error("alias.".$index)
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                    @enderror
                                    @isset($product->product_alias[$index])
                                        <input type="hidden" name="aliasIDs[]" value="{{ $product->product_alias[$index]?->id }}">
                                    @endisset
                                </div>
                            </div>
