...
class LeadsSuperController extends Controller
{
    ...
    {        
        ...
        return view('supervisor_leads.show', [
            "leads" => $leads,
    ...
    public function datatableAjax(Request $request)
    {
        ...
        foreach($leads as $lead){            
            ...
            $filteredRecord["lead"]["href"] = route('supervisor.leads.details',['id' => $lead->id]);
            $filteredRecord["email"] = $lead->email;
            $filteredRecord["phone"] = $lead->phone;
            $filteredRecord["status"] = $lead->status?->name;
            $filteredRecord["agent"] = $lead->agent?->id." - ".$lead->agent?->first_name." ".$lead->agent?->last_name;
            $filteredRecord["actions"]["view"] = route('supervisor.leads.details',['id' => $lead->id]);
            $filteredRecord["actions"]["reassign"] = route('supervisor.leads.reassign',['id' => $lead->id]);
    ...
    public function showCreateForm(){
        ...
        return view('supervisor_leads.create', [
    ...
    public function create(CreateLeadRequest $request)
    {
        ...
        $lead->fk_agent = $request->input("agent");
        ...
        return redirect(route('supervisor.leads.show'))->with('message', 'Lead created successfully');
    }
    ...
    public function showDetailsForm($id, Request $request){
        ...
        return view('supervisor_leads.details', [
            "lead" => $lead,
    ...
    public function updateDetails($id, UpdateDetailsRequest $request){
        ...
        return redirect(route('supervisor.leads.details',['id' => $id]))->with('message', 'Lead updated successfully');
    }
    ...
    public function showUpdateForm($id){
        ...
        return view('supervisor_leads.update', [
            'lead' => $lead,
    ...
    public function showReassignForm($id){

        $lead = CustomersModel::find($id);
        $agents = AgentsModel::orderBy("first_name", "ASC")->orderBy("last_name", "ASC")->get();

        Utils::createLog(
            "The user has entered the form to reassign lead with ID: ".$id,
            "supervisor.leads",
            "show"
        );

        return view('supervisor_leads.reassign', [
            'lead' => $lead,
            "agents" => $agents
        ]);
    }
    ...
    public function reassign($id, Request $request)
    {
        ...
        Utils::createLog(
            "The user has reassigned the lead with ID: ".$id,
            "supervisor.leads",
            "update"
        );

        return redirect(route('supervisor.leads.show'))->with('message', 'Lead reassigned successfully');
    }
